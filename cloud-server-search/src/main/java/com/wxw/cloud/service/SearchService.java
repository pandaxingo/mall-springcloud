...
//    }
    ...
    private List<Brand> getBrandAggResult(Aggregation aggregation) {
        if (aggregation == null || aggregation instanceof UnmappedTerms) {
    ...
    public void saveMQ(Long id) throws IOException {
        Spu spu;
        try {
            spu = this.goodsClient.querySpuById(id);
        } catch (FeignException.NotFound e) {
    ...
    public void initAll() throws IOException {
        ...
        while (true) {
            PageResult<SpuBO> result = this.goodsClient.querySpuByPage(null, true, page, size);
            if (result == null || CollectionUtils.isEmpty(result.getItems())) {
                break;
            }

            for (SpuBO spuBO : result.getItems()) {
                try {
                    Spu spu = new Spu();
