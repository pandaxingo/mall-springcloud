  ...
  <div id="myList" class="myList">
     <ul>
      <li v-for="(item,index) in list" :key="index">
        ...
        <router-link :to="{ path: '/goods/details', query: {productID:item.spuId, skuId:item.id} }">
          <img :src="$imageUrl(item.images)" @error="$imageError" alt />
          <h2>{{formatSubtitle(item)}} &nbsp;&nbsp; 
...
export default {
  ...
  methods:{
    formatSubtitle(item) {
      const subtitle = item && (item.subtitle || item.subTitle) ? (item.subtitle || item.subTitle) : "精选商品";
      return subtitle.length > 5 ? subtitle.substring(0, 5) : subtitle;
    },
    formatTitle(item) {
