...
<template>
   <div id="details">
    ...
    <div class="main">
      <!-- 左侧商品轮播图 -->
      <div class="block">
        <el-carousel height="420px" v-if="carouselImages.length > 1">
          <el-carousel-item v-for="(image,index) in carouselImages" :key="index">
            <img class="detail-image" :src="$imageUrl(image)" @error="$imageError" :alt="spu.title" />
          </el-carousel-item>
        </el-carousel>
        <div class="single-image" v-if="carouselImages.length == 1">
          <img
            class="detail-image"
            :src="$imageUrl(carouselImages[0])"
            @error="$imageError"
            :alt="spu.title"
          />
        </div>
      ...
      <div class="content">
        ...
        <div class="pro-list">
         <br>
          ...
          <p class="price-sum">总计 : {{mainSku.price}}元</p>
        </div>
        <!-- 内容区底部按钮 -->
        <div class="button">
...
export default {
  data() {
    return {
      ...
      paramMap:{},//规格参数
      skus:[], //商品信息
      selectedSku: null, // 从前台商品管理进入时使用的SKU
      spu:{},//SPU信息
      spuDetail:{},// SpU详情信息
    };
  },
  ...
  computed: {
    carouselImages() {
      const detailImages = this.parseImages(this.spuDetail.description);
      if (detailImages.length > 0) {
        return detailImages;
      }
      return (this.skus || [])
        .map(sku => this.parseImages(sku.images))
        .reduce((result, images) => result.concat(images), []);
    },
    mainSku() {
      return this.selectedSku || (this.skus && this.skus[0]) || {};
    }
  },
  ...
  methods: {
    ...
    loadSelectedSku() {
      ...
      if (!skuId) {
        ...
        this.setCartSku(this.mainSku);
        return;
      }
      this.$http.get("/item/sku/" + skuId).then(resp => {
        this.selectedSku = resp.data || null;
        this.setCartSku(this.mainSku);
      }).catch(() => {
        this.selectedSku = null;
        this.setCartSku(this.mainSku);
    ...
    setCartSku(sku) {
      ...
      let mycarts={
          skuId:sku.id,
          image:this.$imageUrl(this.carouselImages[0] || sku.images),
          title:sku.title,
          ownSpec:sku.ownSpec,
          price:sku.price,
          stock:sku.stock,
    ...
    addShoppingCart() {
        if (!this.mainSku.id) {
          this.notifyError("商品信息加载失败，请稍后再试！");
          return;
        }

        const num = Number(this.num || 1);
        ...
        tools.http.get("/auth/verify").then(res=>{
           ...
           console.log(res.data);
            ...
            let cart =carts.id == this.mainSku.id;
            if(cart){
               carts.num=num;
            }else{
                // 没有则新增购物车记录
    ...
    addCollect() {
      ...
      const cart = Object.assign({}, this.mycarts, {
        num: Number(this.num || 1)
      });
      tools.http.get("/auth/verify").then(() => {
        window.localStorage.setItem("BS_BUY_NOW", JSON.stringify(cart));
        this.$router.push({ path: "/confirmOrder", query: { buyNow: 1 } });
        return;
        this.$http.post("/cart", cart).then(res => {
          if (res.status === 201) {
            this.$router.push("/confirmOrder");
            return;
          }
          this.notifyError("立即购买失败，请稍后再试");
        }).catch(err => {
          this.notifyError("立即购买失败，请稍后再试");
          return Promise.reject(err);
        });
...
#details .main .block .el-carousel,
#details .main .block .el-carousel__container,
#details .main .block .el-carousel__item,
#details .main .block .single-image {
  width: 100%;
  height: 420px;
}
#details .main .block .single-image,
...
#details .main .block .detail-image {
  max-width: 100%;
  max-height: 100%;
  width: auto;
  height: auto;
