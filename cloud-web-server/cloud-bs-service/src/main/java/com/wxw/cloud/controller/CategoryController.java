...
public class CategoryController {
    ...
    @ApiOperation("鏍规嵁鍒嗙被鍚嶇О鏌ヨ鍒嗙被")
    @GetMapping("name")
    public ResponseEntity<List<Category>> queryByName(@RequestParam("name") String name) {
        List<Category> list = this.categoryService.queryByName(name);
        if (CollectionUtils.isEmpty(list)) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(list);
