...
public interface IGoodsService {
    ...
    List<SkuVO> querySkusByCid(Long cid);

    List<SkuVO> queryHomeSkus(String area);
    ...
    SkuBO getSkusAndCid3(Integer page, Integer rows, String categoryName, String key);

    SkuBO getSkusByBrand(Long brandId, Integer page, Integer rows);

    PageResult<Sku> querySkuPage(String key, Boolean enable, Integer page, Integer rows, Boolean homeOnly);

    void saveSku(Sku sku);

    void updateSku(Sku sku);

    void updateSkuEnable(Long id, Boolean enable);

    void deleteSku(Long id);

    String saveSkuImage(MultipartFile imageFile);
